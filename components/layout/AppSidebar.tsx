...
export function AppSidebar({ collapsed = false, hidden = false, mobileOpen = false, onToggleCollapse }: AppSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        ...
        !hidden && !mobileOpen && 'max-md:-translate-x-full',
        !hidden && mobileOpen && 'max-md:translate-x-0',
      )}
    >
      <div className="absolute inset-x-0 top-0 h-36 bg-gradient-to-br from-cyan-500/25 via-emerald-400/15 to-amber-300/10" />

      {/* Logo */}
      <div className={cn('relative flex items-center gap-3 border-b border-white/10 py-5', compact ? 'justify-center px-3' : 'px-5')}>
        ...
        <div className={cn('min-w-0 transition-opacity duration-200', compact && 'hidden')}>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={cn('text-xs text-emerald-400 font-medium', compact && 'hidden')}>ระบบทำงานปกติ</span>
        </div>
        <p className={cn('mt-1 text-[10px] text-white/45', compact && 'hidden')}>Live data prototype</p>
      </div>

      {/* Navigation */}
      <nav className="relative flex-1 space-y-1 overflow-y-auto px-3 py-4">
        <p className={cn('mb-3 px-3 text-[10px] font-semibold uppercase tracking-widest text-white/35', compact && 'sr-only')}>เมนูหลัก</p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn('flex-1', compact && 'hidden')}>{item.title}</span>
              {isActive && !compact && <ChevronRight className="h-3.5 w-3.5 text-slate-400" />}
            </Link>
          )
        })}
      </nav>
      ...
      {/* Footer */}
      <div className={cn('relative border-t border-white/10 px-4 py-4', compact && 'px-3')}>
        <div className="rounded-lg border border-white/10 bg-white/[0.06] px-3 py-3">
          <div className={cn('flex items-center gap-2', compact && 'justify-center')}>
            ...
            <p className={cn('text-xs font-semibold text-white/85', compact && 'hidden')}>สำนักงานสาธารณสุข</p>
          </div>
          <p className={cn('mt-1 text-[10px] text-white/45', compact && 'hidden')}>จังหวัดสตูล • ปีงบประมาณ 2568</p>
        </div>
      </div>
    </aside>
  )
